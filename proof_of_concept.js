    ...
    fill(opts.cube)
      ...
      for (let x = 0; x < opts.grid_height; x++) {
            for (let y = 0; y < opts.grid_width; y++) {
                ...
                cubes = int(noise((x) * opts.noise_scale, y * opts.noise_scale) * opts.noise_multiplier) / opts.noise_dampener
                
              for (let i = parseInt(cubes) - 1; i < parseInt(cubes); i++) {
                    draw_block((start_block_x + x*opts.block_size) - y*opts.block_size + opts.x_offset, (start_block_y + x*(opts.block_size/2)) + y*(opts.block_size/2) - int(i)*(opts.block_height) + opts.y_offset, i)
            }
