...
var _require = require("potrace"),
    Potrace = _require.Potrace;

exports.pluginOptionsSchema = function (_ref) {
  var Joi = _ref.Joi;
  return Joi.object({
    maxWidth: Joi.number().default(650).description("The maxWidth in pixels of the div where the markdown will be displayed. This value is used when deciding what the width of the various responsive thumbnails should be."),
    ...
    withWebp: Joi.alternatives().try(Joi.object({
      quality: Joi.number()
    }), Joi.boolean()).default(false).description("Additionally generate WebP versions alongside your chosen file format. They are added as a srcset with the appropriate mimetype and will be loaded in browsers that support the format. Pass true for default support, or an object of options to specifically override those for the WebP files. For example, pass { quality: 80 } to have the WebP images be at quality level 80."),
    withAvif: Joi.alternatives().try(Joi.object({
      quality: Joi.number()
    }), Joi.boolean()).default(false).description("Additionally generate AVIF versions alongside your chosen file format. They are added as a srcset with the appropriate mimetype and will be loaded in browsers that support the format. Pass true for default support, or an object of options to specifically override those for the AVIF files. For example, pass { quality: 80 } to have the AVIF images be at quality level 80."),
    tracedSVG: Joi.alternatives().try(Joi.boolean(), Joi.object({
      turnPolicy: Joi.string().valid( // this plugin also allow to use key names and not exact values
      "TURNPOLICY_BLACK", "TURNPOLICY_WHITE", "TURNPOLICY_LEFT", "TURNPOLICY_RIGHT", "TURNPOLICY_MINORITY", "TURNPOLICY_MAJORITY", // it also allow using actual policy values
      Potrace.TURNPOLICY_BLACK, Potrace.TURNPOLICY_WHITE, Potrace.TURNPOLICY_LEFT, Potrace.TURNPOLICY_RIGHT, Potrace.TURNPOLICY_MINORITY, Potrace.TURNPOLICY_MAJORITY).default(Potrace.TURNPOLICY_MAJORITY),
      turdSize: Joi.number().default(100),
      alphaMax: Joi.number(),
      optCurve: Joi.boolean().default(true),
      optTolerance: Joi.number().default(0.4),
      threshold: Joi.alternatives().try(Joi.number().min(0).max(255), Joi.number().valid(Potrace.THRESHOLD_AUTO)).default(Potrace.THRESHOLD_AUTO),
    ...
    loading: Joi.string().valid("lazy", "eager", "auto").default("lazy").description("Set the browser\u2019s native lazy loading attribute. One of lazy, eager or auto."),
    decoding: Joi.string().valid("async", "sync", "auto").default("async").description("Set the browser\u2019s native decoding attribute. One of async, sync or auto."),
    disableBgImageOnAlpha: Joi.boolean().default(false).description("Images containing transparent pixels around the edges results in images with blurry edges. As a result, these images do not work well with the \u201Cblur up\u201D technique used in this plugin. As a workaround to disable background images with blurry edges on images containing transparent pixels, enable this setting."),
    disableBgImage: Joi.boolean().default(false).description("Remove background image and its\u2019 inline style. Useful to prevent Stylesheet too long error on AMP."),
